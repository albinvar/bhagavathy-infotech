...
class MY_Model extends CI_Model
{
    ...
    /**
     * Fetch an array of records based on an array of field values.
     */
    public function get_many_in($field,$values)
    {
       //$check= "FIND_IN_SET($field,'$values')";
        $this->_database->where_in($field,$values);
     ...
     * Update many records, based on an array of primary values.
     */
    public function update_many_in($field,$primary_values, $data, $skip_validation = FALSE)
    {
        $data = $this->trigger('before_update', $data);
        ...
        if ($data !== FALSE)
        {
            $result = $this->_database->where_in($field, $primary_values)
                ->set($data)
                ->update($this->_table);
...
public function update_status_by()
   {
       $args = func_get_args();
       $data = array_pop($args);
     ...
     */
    public function validate($data,$module="")
    {
        ...
        if(!empty($module))
        {
            $this->validate=$this->validate[$module];
        }
